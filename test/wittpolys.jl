...
function ghostpoly(x::Vector,n=length(x)) where T <: RingElem
...
function ghostmap(w::WittVector, n) where T <: RingElem
	return ghostpoly(w.xcoords, n)
end
function ghostmap(w::TruncatedWittVector, n) where T <: RingElem
	return ghostpoly(w.xcoords, n)
end
