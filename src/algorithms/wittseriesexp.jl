#=
function calc_ring(W::BigWittRing{TT}) where TT <: RingElem
	R=base_ring(W)
	n=W.prec
	if parentmodule(typeof(R))=Nemo
		return Nemo.PowerSeriesRing(R,n,"T",model=:capped_absolute)
	else 
		return AbstractAlgebra.PowerSeriesRing(R,n,"T", model="capped_absolute")
	end
end
=#
